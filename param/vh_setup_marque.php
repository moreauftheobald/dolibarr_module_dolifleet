		...
		if (($action == 'confirmnew') || ($action == 'confirmedit' && !empty($rowid)) || ($action == 'confirmdelete' && !empty($rowid))) {
			...
			if (empty($error)) {
				if ($action == 'confirmnew') {
					$sql = "INSERT INTO " . $db->prefix() . "c_dolifleet_vehicule_mark (entity,code, label, active, date_creation) VALUES (";
					$sql .= ((int) $conf->entity) . ",";
					$sql .= "'" . $db->escape($code) . "',";
					$sql .= "'" . $db->escape($label) . "',";
					$sql .= ((int) $active) . ",";
					...
					$sql = "UPDATE " . $db->prefix() . "c_dolifleet_vehicule_mark SET ";
					$sql .= "code = '" . $db->escape($code) . "', ";
					$sql .= "label = '" . $db->escape($label) . "', ";
					$sql .= "active = " . ((int) $active) . " ";
					$sql .= "WHERE rowid = " . ((int) $rowid);
				} elseif ($action == 'confirmdelete') {
					$sql = "DELETE FROM " . $db->prefix() . "c_dolifleet_vehicule_mark WHERE rowid = " . ((int) $rowid);
				}
				$res = $db->query($sql);
				if (!$res) {
					$error++;
...
$sql .= "FROM ".$db->prefix()."c_dolifleet_vehicule_mark as p ";
...
if ($action=='delete' && !empty($rowid)) {
	...
	$formconfirm = $form->formconfirm(dol_escape_htmltag($_SERVER["PHP_SELF"]), $langs->trans('DeleteMarque'), $langs->trans('DeleteMarqueQuestion'), 'confirmdelete', $formquestion, 'yes', 1);
} elseif ($action =='new') {
	...
	$formconfirm = $form->formconfirm(dol_escape_htmltag($_SERVER["PHP_SELF"]), $langs->trans('NewMarque'), '', 'confirmnew', $formquestion, 'yes', 1, 0, 700);
	...
	$formconfirm = $form->formconfirm(dol_escape_htmltag($_SERVER["PHP_SELF"]), $langs->trans('EditMarque'), '', 'confirmedit', $formquestion, 'yes', 1, 0, 700);
}
