		...
		if (($action == 'confirmnew') || ($action == 'confirmedit' && !empty($rowid)) || ($action == 'confirmdelete' && !empty($rowid))) {
			...
			if (empty($error)) {
				if ($action == 'confirmnew') {
					...
					$sql = "DELETE FROM " . $db->prefix() . "c_dolifleet_contract_type WHERE rowid = " . ((int) $rowid);
				}
				$res = $db->query($sql);
				if (!$res) {
					$error++;
...
if ($action=='delete' && !empty($rowid)) {
	...
	$formconfirm = $form->formconfirm(dol_escape_htmltag($_SERVER["PHP_SELF"]), $langs->trans('DeleteTypect'), $langs->trans('DeleteTypectQuestion'), 'confirmdelete', $formquestion, 'yes', 1);
} elseif ($action =='new') {
	...
	$formconfirm = $form->formconfirm(dol_escape_htmltag($_SERVER["PHP_SELF"]), $langs->trans('NewTypect'), '', 'confirmnew', $formquestion, 'yes', 1, 0, 700);
	...
	$formconfirm = $form->formconfirm(dol_escape_htmltag($_SERVER["PHP_SELF"]), $langs->trans('EditTypect'), '', 'confirmedit', $formquestion, 'yes', 1, 0, 700);
}
